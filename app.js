// Toilet Paper Pack Generator - fullscreen version with debug console
...
function init3D() {
  ...
  if (typeof THREE === "undefined") {
    logDebug("ERROR: THREE is not defined inside init3D");
    return;
  }
  ...
  if (!height || height < 50) {
    height = 450;
    logDebug("canvas-container height was small, using fallback 450px");
  }
...
function buildPack() {
  ...
  const packWidthLocal = intRows * rollDiameter;
  const packDepthLocal = intRollsPerRow * rollDiameter;

  const startX = -packWidthLocal / 2 + rollRadius;
  const startZ = -packDepthLocal / 2 + rollRadius;
  ...
  const boxGeom = new THREE.BoxGeometry(packWidthLocal, packHeight, packDepthLocal);
...
function applyPackRotation() {
  if (!packGroup || typeof THREE === "undefined") return;
...
function applyCameraControls() {
  if (!camera || !controls || !targetCenter || typeof THREE === "undefined") return;
  ...
  const theta = THREE.MathUtils.degToRad(angleXDeg);
  const phi = THREE.MathUtils.degToRad(angleYDeg);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  logDebug("DOMContentLoaded inside app.js");
  logDebug("THREE visible at start?", { hasTHREE: typeof THREE !== "undefined" });

  if (typeof THREE === "undefined") {
    logDebug("ERROR: THREE is undefined at startup. Check CDN loading or network.");
